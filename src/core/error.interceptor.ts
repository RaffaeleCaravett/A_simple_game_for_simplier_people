      ...
      catchError((err: any) => {
        setTimeout(() => {
          if (err instanceof HttpErrorResponse) {
            if (
              err?.error?.message ==
              'Hai già una richiesta attiva inviata a questo utente'
            ) {
            ...
            if (
              ((err.error &&
                ...
                (err.error &&
                  err.error.message &&
                  err.error.message == 'Il token non è valido.') ||
                err.status === 401 ||
                err.status === 403) &&
              !(
                err?.error?.message ==
              ...
              if (refreshToken) {
                this.authService.verifyRefreshToken(refreshToken!).subscribe({
                  next: (tokens: any) => {
              ...
              let error = null;
              if (
                null != err?.error?.messageList &&
                err?.error?.messageList.length > 0
              ) {
