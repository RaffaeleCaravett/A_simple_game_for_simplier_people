...
export class ChatService {
  ...
  readMessages(chatId: number) {
    return this.http.get(
      environment.API_URL + this.messaggio + this.read + `?chatId=${chatId}`
    );
  }

  getAllChatsByUserId(userId: number) {
    return this.http.get(
      environment.API_URL + this.chat + this.params + `?userId=${userId}`
    );
  }
  /* TO IMPLEMENT
  ...
  */
  ...
  getAvailableUsersForChat() {
    return this.http.get(
      environment.API_URL + this.chat + '/availableContacts'
    );
  }

  getChatMenu(chatId: number) {
    return this.http.get(
      environment.API_URL + this.chat + '/options/' + chatId
    );
  }

  patchChat(chatId: number, chatDTO: ChatDTO) {
    return this.http.patch(
      environment.API_URL + this.chat + '/' + chatId,
      chatDTO
    );
  }
  ...
  changeChatImage(chatId: number, chatImage: File) {
    ...
    return this.http.put(
      environment.API_URL + this.chat + this.updateImage + '/' + chatId,
      formData
    );
  }
  ...
  uploadMessageImages(files: File[], messageId: number) {
    ...
    return this.http.post(
      environment.API_URL + this.messageImage + '/' + messageId,
      formData
    );
  }
