...
export class LoginComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.formsService.requestLoginCode.subscribe((data: any) => {
      if (
        ...
        this.interval = setTimeout(() => {
          ...
          this.formsService
            .clearCode(this.loginForm.get('email')?.value)
            .subscribe({
              next: (data: any) => {
  ...
  ngOnInit(): void {
    ...
    this.codeForm = new FormGroup({
      ...
      secondLetter: new FormControl('', [
        Validators.required,
        Validators.maxLength(1),
      ]),
      thirdLetter: new FormControl('', [
        Validators.required,
        Validators.maxLength(1),
      ]),
      fourthLetter: new FormControl('', [
        Validators.required,
        Validators.maxLength(1),
      ]),
      fifthLetter: new FormControl('', [
        Validators.required,
        Validators.maxLength(1),
      ]),
      sixthLetter: new FormControl('', [
        Validators.required,
        Validators.maxLength(1),
      ]),
    });
  ...
  login() {
    ...
    if (
      ...
      this.toastr.show(
        "Ti abbiamo già mandato un codice per mail, inseriscilo qui. E' valido per 2 minuti."
      );
    else if (
  ...
  switchRoute(route: string) {
    route == 'forms'
      ...
      : this.router.navigate(['forms/signup']);
  }
  ...
  validateCode() {
    if (this.codeForm.valid) {
      let code =
        this.codeForm.get('firstLetter')?.value +
        this.codeForm.get('secondLetter')?.value +
        this.codeForm.get('thirdLetter')?.value +
        this.codeForm.get('fourthLetter')?.value +
        this.codeForm.get('fifthLetter')?.value +
        this.codeForm.get('sixthLetter')?.value;
      this.formsService
        .verifyCode(this.loginForm.get('email')?.value, code, true)
        .subscribe({
          next: (data: any) => {
            if (data) {
              this.previousEmail = '';
  ...
  updateCodeForm() {
    if (this.codeForm.get('firstLetter')?.value == '') {
      ...
      if (this.codeForm.get('firstLetter')?.value.length > 1)
        this.codeForm
          .get('firstLetter')
          ?.setValue(this.codeForm.get('firstLetter')?.value.substring(0, 1));
    }
    ...
    if (this.codeForm.get('secondLetter')?.value == '') {
      ...
      if (this.codeForm.get('secondLetter')?.value.length > 1)
        this.codeForm
          .get('secondLetter')
          ?.setValue(this.codeForm.get('secondLetter')?.value.substring(0, 1));
    }
    ...
    if (this.codeForm.get('thirdLetter')?.value == '') {
      ...
      if (this.codeForm.get('thirdLetter')?.value.length > 1)
        this.codeForm
          .get('thirdLetter')
          ?.setValue(this.codeForm.get('thirdLetter')?.value.substring(0, 1));
    }
    ...
    if (this.codeForm.get('fourthLetter')?.value == '') {
      ...
      if (this.codeForm.get('fourthLetter')?.value.length > 1)
        this.codeForm
          .get('fourthLetter')
          ?.setValue(this.codeForm.get('fourthLetter')?.value.substring(0, 1));
    }
    if (this.codeForm.get('fifthLetter')?.value == '') {
      ...
      if (this.codeForm.get('fifthLetter')?.value.length > 1)
        this.codeForm
          .get('fifthLetter')
          ?.setValue(this.codeForm.get('fifthLetter')?.value.substring(0, 1));
    }

    if (this.codeForm.get('sixthLetter')?.value.length > 1)
      this.codeForm
        .get('sixthLetter')
        ?.setValue(this.codeForm.get('sixthLetter')?.value.substring(0, 1));
  ...
  async signInWithGoogle() {
    try {
      ...
      if (errorCode === 'auth/network-request-failed') {
        this.toastr.warning(
          "C'è stato un problema di rete. Per favore verifica la tua connessione."
        );
      } else {
        this.toastr.error(
